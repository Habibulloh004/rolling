...
export default function OrderItemComponent({
  ...
  const getCandidateOrderIds = (order) =>
    [
  ...
  const getOrderCreatedAtValue = (order) => {
    const value = new Date(order?.created_at || order?.createdAt || 0).getTime();
    return Number.isFinite(value) ? value : 0;
  };

  const getOrderPriorityScore = (order) => {
    const normalizedStatus = getNormalizedStatus(order);
    if (["cancel", "cancelled", "canceled", "rejected"].includes(normalizedStatus)) return 3;
    if (["finished", "delivered", "completed", "done", "taken", "closed"].includes(normalizedStatus)) return 2;
  ...
  const getLocalOrder = (orderId) => {
    try {
      ...
      const normalizedId = normalizeOrderToken(orderId);
      return list
        .filter((item) => getCandidateOrderIds(item).includes(normalizedId))
        .reduce(selectPreferredOrder, null);
    } catch {
      return null;
    }
  };
  ...
  const fetchAddress = useCallback(async () => {
    try {
      ...
      const candidateOrderId = orderPay?.order_id || param?.id;
  ...
  useEffect(() => {
    const normalizeId = (value) => normalizeOrderToken(value).toLowerCase();
            ...
            <div className="flex flex-col gap-y-4">
              ...
              {orderData?.payed_bonus && orderData?.payed_bonus != 0 ? (
                <div className="w-full flex justify-between">
                  ...
                  <p className="font-normal textNormal2 leading-7 text-[#2E2E2E]">
                    {formatNumber(Number(orderData?.payed_bonus || 0) / 100)}{" "}
                    {all("sum")}
                  </p>
                </div>
              ) : null}
