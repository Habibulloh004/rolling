"use client";

import CustomImage from "@/components/shared/customImage";
import { cn, formatNumber, posterUrl, truncateText } from "@/lib/utils";
import { useOrderStore, useProductStore, useStore } from "@/store";
import { Heart, Minus, Plus } from "lucide-react";
import { useTranslations } from "next-intl";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import { memo, useCallback, useMemo, useRef } from "react";

const Card = memo(({
  item,
  localizedName,
  localizedDesc,
  locale,
  defaultHref = "/web/category",
  ...props
}) => {
  const router = useRouter();
  const all = useTranslations("All");
  const isFavorite = useStore((state) => state.favorites.some((f) => f.product_id === item.product_id));
  const setFavorites = useStore((state) => state.setFavorites);
  const { photo, price } = props;
  const productPhoto = item?.photo_origin || photo || item?.photo;
  const findProduct = useProductStore((state) => state.products.find((p) => p.product_id == item.product_id) || null);
  const setProducts = useProductStore((state) => state.setProducts);
  const incrementCount = useProductStore((state) => state.incrementCount);
  const decrementCount = useProductStore((state) => state.decrementCount);
  const hasPayment = useOrderStore((state) => !!(state.paymentData && state.paymentData.payment_id));
  const prefetchedHrefRef = useRef("");
  const searchParams = useSearchParams();
  const resolvedHref = useMemo(() => {
    if (!defaultHref || !defaultHref.includes("/branch/")) return defaultHref;
    const spot = searchParams.get("spot");
    if (!spot) return defaultHref;
    const qs = new URLSearchParams();
    for (const key of ["spot", "table_id", "table_num", "service"]) {
      const val = searchParams.get(key);
      if (val) qs.set(key, val);
    }
    const str = qs.toString();
    return str ? `${defaultHref}?${str}` : defaultHref;
  }, [defaultHref, searchParams]);

  const handleAddProduct = useCallback(() => {
    setProducts(item);
  }, [setProducts, item]);
  const handleIncrementCount = useCallback(() => {
    incrementCount(item?.product_id);
  }, [incrementCount, item?.product_id]);
  const handleDecrementCount = useCallback(() => {
    decrementCount(item?.product_id);
  }, [decrementCount, item?.product_id]);

  const handleAddFavorite = useCallback((product) => {
    const favorites = useStore.getState().favorites;
    const favorite = favorites.find(
      (f) => f.product_id === product.product_id
    );
    let updatedFavorites;

    if (!favorite) {
      updatedFavorites = [...favorites, product];
    } else {
      updatedFavorites = favorites.filter(
        (f) => f.product_id !== product.product_id
      );
    }

    setFavorites(updatedFavorites);
    localStorage.setItem("isFavorites", JSON.stringify(updatedFavorites));
  }, [setFavorites]);

  const prefetchProductRoute = useCallback(() => {
    if (!resolvedHref || prefetchedHrefRef.current === resolvedHref) {
      return;
    }

    prefetchedHrefRef.current = resolvedHref;
    router.prefetch(resolvedHref);
  }, [resolvedHref, router]);
  return (
    <div className="relative w-full bg-white rounded-md p-2 space-y-2 h-full flex flex-col justify-between">
      {/* Wrap the clickable area with Link */}
      <Link
        href={resolvedHref}
        prefetch={false}
        onMouseEnter={prefetchProductRoute}
        onTouchStart={prefetchProductRoute}
        onFocus={prefetchProductRoute}
        className="block space-y-2"
      >
        <div className="relative border-border border w-full aspect-square overflow-hidden rounded-sm">
          <CustomImage
            src={productPhoto ? `${posterUrl}${productPhoto}` : "/empty.jpg"}
            cacheKey={`product:${item?.product_id}`}
            className="w-full h-full"
            alt={`${localizedName}`}
            sizes="(max-width: 640px) 50vw, (max-width: 1024px) 25vw, (max-width: 1536px) 20vw, 16vw"
          />
        </div>
        <div className="space-y-1">
          <h1 className="textSmall2 font-bold">
            {truncateText(localizedName, 40)}{" "}
          </h1>
          <h1 className="textSmall1 font-[400]">
            {truncateText(localizedDesc, 50)}{" "}
          </h1>
          <p className="textSmall2 font-bold">
            {price ? formatNumber(price) : 0} сум
          </p>
        </div>
      </Link>
      {/* Favorite Button */}
      {!resolvedHref?.startsWith(`/${locale}/branch`) && (
        <button
          aria-label={`shcard heart`}
          onClick={(e) => {
            e.stopPropagation();
            handleAddFavorite(item);
          }}
          className="absolute right-3 top-1 md:top-2 rounded-full bg-white p-1 shadow-sm transition-colors hover:bg-gray-100"
        >
          <Heart
            className={cn(
              "h-6 w-6",
              isFavorite ? "fill-[#43674E] text-[#43674E]" : "text-gray-400"
            )}
          />
        </button>
      )}
      {!findProduct ? (
        <div className="flex justify-end items-center gap-2">
          <button
            aria-label={`shcard plus`}
            disabled={hasPayment}
            onClick={handleAddProduct}
            className="rounded-md px-3 py-[10px] bg-primary active:bg-primary-modal text-white text-xs"
          >
            {all("add")}
          </button>
        </div>
      ) : (
        <div className="flex justify-end items-center gap-2">
          <div className="max-sm:w-full flex justify-around sm:justify-center items-center gap-1 bg-primary rounded-md">
            <button
              aria-label={`shcard plus2`}
              disabled={hasPayment}
              onClick={handleIncrementCount}
              className="max-sm:w-full rounded-l-md p-2 bg-primary active:bg-gradient-to-r active:from-white/20 active:to-primary"
            >
              <Plus
                className="text-white max-md:w-4 max-md:h-4 w-5 h-5"
                size={18}
              />
            </button>
            <span className="font-bold text-[12px] md:textSmall4 text-white min-w-6 text-center">
              {findProduct.count >= 10
                ? findProduct.count
                : `0${findProduct.count}`}
            </span>
            <button
              aria-label={`shcard minus`}
              disabled={hasPayment}
              onClick={handleDecrementCount}
              className="max-sm:flex justify-center items-center max-sm:w-full rounded-r-md p-2 bg-primary active:bg-gradient-to-l active:from-white/20 active:to-primary"
            >
              <Minus className="text-white max-md:w-4 max-md:h-4 w-5 h-5" />
            </button>
          </div>
        </div>
      )}
    </div>
  );
});

Card.displayName = "Card";

export default Card;
